...
# Primary/default path: free OpenRouter models first, paid model only as a backstop.
LLM_MODEL = os.environ.get("LLM_MODEL", "openrouter/qwen/qwen3-coder:free")
    ...
    for spec in _build_llm_candidate_specs(resolved_llm):
        ...
        while True:
            ...
            try:
                ...
                remaining = float(resolved_llm.get("max_total_seconds") or SCIENCE_CLASSIFIER_MAX_TOTAL_SECONDS) - (
                    time.monotonic() - started
                )
                retry_sleep = _free_llm_retry_sleep_seconds(
                    exc, retry_index=free_rate_retries, remaining_seconds=remaining
                )
                if (
                    _is_free_llm_spec(spec)
                    and _is_retryable_free_llm_rate_limit(exc)
                    and free_rate_retries < max(0, LLM_FREE_RATE_LIMIT_RETRIES)
                    and retry_sleep > 0
                ):
                    free_rate_retries += 1
                    attempt["retrying_same_model"] = True
                    attempt["retry_sleep_seconds"] = round(retry_sleep, 3)
                    _log_llm_attempt(attempt)
                    attempts.append(attempt)
...
def _is_free_llm_spec(spec: Dict[str, str]) -> bool:
    return ":free" in (spec.get("model") or "").lower()


def _extract_llm_retry_after_seconds(error: Exception) -> Optional[float]:
    text = str(error)
    patterns = (
        r'"retry_after_seconds_raw"\s*:\s*([0-9]+(?:\.[0-9]+)?)',
...
def _free_llm_retry_sleep_seconds(error: Exception, *, retry_index: int, remaining_seconds: float) -> float:
    if remaining_seconds <= 2:
        return 0.0
    retry_after = _extract_llm_retry_after_seconds(error)
    backoff = retry_after if retry_after is not None else float(2 ** max(0, retry_index))
    return max(0.0, min(backoff, LLM_FREE_RATE_LIMIT_MAX_SLEEP, remaining_seconds - 1.0))


def _is_retryable_free_llm_rate_limit(error: Exception) -> bool:
